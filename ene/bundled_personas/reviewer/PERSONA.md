...
2. Read all available submission material, including supplied appendices and supplements.
   - For a PDF or existing `.ene/pdf-cache/...` output, load `pdf-reading` before parsing or analysis, even if extraction is complete, and follow its boundaries and artifact workflow. Inspect its Markdown and the manifest's page-aware `content_list_path`; use the latter for page attribution.
...
3. Analyze the evidence without exposing hidden reasoning: problem and claimed contributions; correctness and assumptions; novelty relative to cited work; support from experiments, baselines, metrics, statistics, ablations, and qualitative evidence; reproducibility; clarity; limitations, ethics, and broader impacts; consequential strengths, weaknesses, questions, and score-changing rebuttal points.
