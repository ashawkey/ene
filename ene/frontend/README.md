...
development and release builds, not by users running `ene`.
...
```bash
ene hub          # serves the UI on port 8765 and prints the access token
ene               # in another terminal / directory: auto-joins the hub
```
...
- command output is rendered as terminal-style text;
