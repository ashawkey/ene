...
def read_persona(persona_dir: str | Path, source: str = "local") -> PersonaInfo:
    ...
    for line in template.splitlines():
        ...
        if marker not in _MARKERS:
            raise ValueError(f"unknown marker '{{{{ene:{marker}}}}}'")
        if len(matches) != 1 or line.strip() != match.group(0):
            raise ValueError(f"marker '{{{{ene:{marker}}}}}' must occupy its own line")
        if marker in seen:
            raise ValueError(f"duplicate marker '{{{{ene:{marker}}}}}'")
        seen.add(marker)
...
def discover_personas(
    ...
    locations = [
        ...
        (base / ".ene" / "personas", "project"),
    ]
    if base.resolve() != home.resolve():
        locations.append((home / ".ene" / "personas", "personal"))
...
def filter_persona_skills(
    ...
    """Return skills this persona advertises through ``{{ene:skills}}``.
