"""Agent-side client that links a terminal ``ene`` to the shared web hub.

A terminal ``ene`` process stays terminal-first. This client runs in a daemon
thread, connects to the hub's loopback ``/internal/agent`` endpoint, registers
a session, forwards every :class:`~ene.utils.io.EventHub` event, and injects
