"""Skill discovery, parsing, and registry formatting for ene.
...
not enforced: all ordinary ene tools execute automatically, so a skill cannot
...
Skills are discovered from ene's bundled skill directory and ``.ene/skills``
...
match the installed ene version; project skills take precedence over personal
skills. Skills for other agents can still be used by giving ene their paths
explicitly.
"""
...
# instructions and resources always match the installed ene version.
...
def discover_skills(
    ...
    Bundled skills are searched first so stale project copies left by older ene
...
def build_skills_prompt_section(skills: dict[str, dict]) -> str:
    ...
    lines = [
        "## Skills",
        "",
        "Before acting, match the request against the skills below and follow matching",
        "skills' instructions. Reuse full instructions already in context; call",
        "**load_skill** only when they are missing or incomplete (for example, after",
        "context compaction). Do not reload a skill merely because it matches a new turn.",
        "Always apply **skill-creator** before creating or modifying a skill, using",
        "the same loading rule. Resolve referenced",
...
def load_skill_tools(skill_dir: str | Path) -> list[dict[str, Any]]:
    ...
    module_name = f"ene_skill_tools_{uuid.uuid4().hex}"
