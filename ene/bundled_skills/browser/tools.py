...
from ene.tools import ToolCallDescription
...
def _discover_cdp_url(explicit: str | None) -> str:
    if explicit:
        return explicit
    configured = os.environ.get("ENE_BROWSER_CDP_URL")
    if configured:
        return configured
...
(() => {
  ...
  window.__eneBrowserElements = interactive;
...
def _element_geometry(connection: CDPConnection, index: int) -> dict[str, Any]:
    script = f"""
(() => {{
  const el = window.__eneBrowserElements?.[{index - 1}];
...
def browser_scroll(
    ...
    target = "window" if index is None else f"window.__eneBrowserElements?.[{index - 1}]"
...
def browser_stop(executor) -> dict[str, Any]:
    """Detach Ene from Chrome without closing the user's browser."""
    connection = getattr(executor, "_browser_connection", None)
...
TOOLS = [
    ...
    },
    {
        ...
        "schema": _schema(
            "browser_stop",
            "Detach Ene from Chrome without closing the user's browser or tabs.",
            {},
        ),
    },
]
