...
<skill-name>/
├── SKILL.md          # required
├── tools.py          # optional ene extension
├── scripts/          # optional executable helpers
├── references/       # optional documentation
└── assets/           # optional templates and static files
```

Create project skills under `.ene/skills/<skill-name>/` unless the user requests
...
has no safe, unambiguous default task, omit that section: ene will load the
skill and ask the user what to do. An ordinary `## Workflow` section does not by
itself declare a default invocation.

With trailing task context, ene loads the skill and applies its instructions to
...
Resolve resource paths against the absolute skill directory shown when ene loads
the skill.
