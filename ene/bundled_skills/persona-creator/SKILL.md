...
description: Create, revise, or validate custom Ene personas and their tool access.
---

# Persona Creator

Create one focused persona at `.ene/personas/<name>/PERSONA.md`.
...
2. Choose a name of 1-64 lowercase alphanumeric segments separated by single hyphens. `reload` and bundled names such as `coder`, `chat`, and `reviewer` cannot be overridden; use a new name when customizing a bundled persona.
...
5. Select the skills advertised through `{{ene:skills}}`: list only the bundled skills relevant to the role, and enable local skills only when project/personal skills should be discoverable.
...
`skills` must contain `bundled`, either `all` or an explicit list of bundled skill names advertised to the persona, and `local`, a boolean controlling whether project and personal `.ene/skills` entries are advertised. Use `bundled: []` and `local: false` for no skill metadata. This only limits `{{ene:skills}}`; users can still explicitly load a discovered skill with `/skills <name>`.

## Runtime markers

Available whole-line markers are:

- `{{ene:autonomous-mode}}`: autonomous execution rules; empty in interactive mode.
- `{{ene:skills}}`: skills selected by the persona's `skills` policy when `load_skill` is allowed.
- `{{ene:project-instructions}}`: the working directory's `AGENTS.md`, when present.
- `{{ene:current-context}}`: working directory, operating system, and Git branch/status.
...
```bash
python -c "from ene.personas import read_persona; print(read_persona(r'.ene/personas/<name>'))"
```

If runtime marker behavior matters, also render it with a representative context:

```bash
python -c "from ene.personas import PersonaContext, read_persona; p=read_persona(r'.ene/personas/<name>'); print(p.build(PersonaContext(work_dir='.')))"
```

Report the created path and validation run. Tell the user to run `/persona reload` in an active ene session; changing the active persona restarts the conversation.
