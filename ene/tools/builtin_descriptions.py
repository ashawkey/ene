"""Call descriptions owned by ene's built-in tools."""
...
def _describe_start_process(args: dict[str, Any]) -> ToolCallDescription:
    primary = args.get("label") or args["command"]
    return _description(
        "start_process",
        quote_tool_call_value(primary),
...
def _describe_process_output(result: dict[str, Any]) -> str:
    ...
    for process in processes:
        ...
        display = process.get("label") or process.get("command", "")
