...
def _process_tree_alive(proc: subprocess.Popen) -> bool:
    """Return whether *proc* or its process group still has live members."""
    proc.poll()  # Reap the leader so it does not keep an otherwise-empty group alive.
...
def _terminate_process(proc: subprocess.Popen, *, force: bool = False) -> None:
    """Terminate *proc* and its child process tree (best-effort)."""
    try:
        if sys.platform == "win32":
            subprocess.run(
