...
from ene.providers.auth import CredentialStore, OAuthCredential
from ene.providers.openai_codex import OpenAICodexProvider, _build_body
from ene.providers.openai_codex_oauth import (
    JWT_CLAIM_PATH,
    _exchange_code,
    _authorization_url,
    _create_pkce,
...
def test_oauth_code_exchange_builds_stored_credential(monkeypatch):
    ...
    monkeypatch.setattr("ene.providers.openai_codex_oauth.httpx.post", post)
...
def test_device_login_wait_can_be_cancelled(monkeypatch):
    ...
    monkeypatch.setattr("ene.providers.openai_codex_oauth.httpx.post", post)
    monkeypatch.setattr(
        "ene.providers.openai_codex_oauth.webbrowser.open", lambda url: True
    )
...
def test_login_removes_credentials_when_cancelled_during_write(monkeypatch):
    ...
    monkeypatch.setattr(
        "ene.providers.openai_codex.login_openai_codex",
...
def test_codex_incomplete_response_does_not_dispatch_partial_tool_call(monkeypatch, tmp_path):
    events = [{
        "type": "response.incomplete",
        "response": {
            "status": "incomplete",
...
def test_expired_oauth_token_is_refreshed_once(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("ene.providers.openai_codex.refresh_openai_codex", refresh)
