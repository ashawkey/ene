...
def test_update_editable_install_runs_git_pull(monkeypatch, tmp_path):
    ...
    distributions = []
    monkeypatch.setattr(
        cli, "distribution", lambda name: distributions.append(name) or FakeDistribution(direct_url)
    )
    monkeypatch.setattr(
        cli.subprocess,
        "run",
    ...
    assert distributions == ["ene-agent"]
...
def test_update_regular_install_reinstalls_from_source(monkeypatch):
    ...
    assert cli.cmd_update() == 0
    assert commands == [
        [cli.sys.executable, "-m", "pip", "uninstall", "-y", "ene-agent"],
        [
            cli.sys.executable,
            "-m",
            "pip",
