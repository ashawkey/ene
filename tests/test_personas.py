...
def test_bundled_personas_are_declarative():
    ...
    assert set(personas) >= {"coder", "chat", "reviewer", "orchestrator"}
    ...
    assert personas["chat"].tools == frozenset({"web_search", "web_fetch"})
    assert personas["chat"].bundled_skills == frozenset()
    assert personas["chat"].local_skills is False
...
def test_read_persona_validates_markers(tmp_path):
    path = _write_persona(tmp_path, "bad", body="Before {{ene:current-context}} after")
...
def test_render_expands_markers_once(tmp_path):
    work = tmp_path / "work"
    work.mkdir()
    (work / "AGENTS.md").write_text("Keep this literal: {{ene:skills}}", encoding="utf-8")
    path = _write_persona(
        tmp_path / "personas",
        "custom",
        tools="[]",
    ...
    assert "Keep this literal: {{ene:skills}}" in prompt
...
def test_local_project_instructions_can_import_root(tmp_path):
    ...
    (work / ".ene" / "AGENTS.md").write_text(
        "@AGENTS.md\n\nLocal instructions.", encoding="utf-8"
    )
    path = _write_persona(
        tmp_path / "personas",
        "custom",
        body="{{ene:project-instructions}}",
    )
...
def test_local_project_instructions_replace_root(tmp_path):
    ...
    (work / ".ene").mkdir(parents=True)
    (work / "AGENTS.md").write_text("Root instructions.", encoding="utf-8")
    (work / ".ene" / "AGENTS.md").write_text("Local only.", encoding="utf-8")
    path = _write_persona(
        tmp_path / "personas",
        "custom",
        body="{{ene:project-instructions}}",
...
def test_bundled_name_cannot_be_shadowed(tmp_path, monkeypatch):
    ...
    _write_persona(project / ".ene" / "personas", "coder", description="Project")
...
def test_project_persona_shadows_personal(tmp_path, monkeypatch):
    ...
    _write_persona(project / ".ene" / "personas", "custom", description="Project")
    _write_persona(home / ".ene" / "personas", "custom", description="Personal")
