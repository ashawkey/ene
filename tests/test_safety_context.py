...
import ene.tools.results as tool_results
...
def test_actual_failed_exec_capture_is_compacted_and_persisted(tmp_path):
    ...
    artifact = (
        tmp_path / ".ene" / "tool-results" / "test"
...
def test_unparseable_arguments_still_answer_their_tool_call(tmp_path):
    ...
    results, so a call ene cannot parse is answered with an error result and
...
def test_stream_is_closed_when_consumption_is_cancelled(monkeypatch):
    ...
    monkeypatch.setattr("ene.backend.run_interruptible", interruptible)
    agent = NS(
        console=_Console(),
        cancellation=None,
        show_thinking=False,
